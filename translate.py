...
def read_prompt(step_heading_prefix: str) -> str:
    ...
    if _CACHED_STEP_BLOCKS is None:
        ...
        _CACHED_STEP_BLOCKS = extract_fenced_code_blocks(markdown_text)

    for block in _CACHED_STEP_BLOCKS:
        if block.lstrip().startswith(step_heading_prefix):
            return block.strip()
...
def step1(llm_client, source_text, reference):
    ...
    prompt = read_prompt("### Step 1:")
    print_header(prompt)
    prompt = prompt.format(source_text=source_text.rstrip(), reference=reference.rstrip())
    return llm_client.call(prompt)

def get_result(history):
    """Extract final translation result from LLMClient history."""
    return extract_fenced_code_blocks(history[-1]["content"])[-1]
...
def translate(client, target_lang):
    """Run the complete 4-stage structured translation process."""
    llm_client = client.copy()

    prompt = read_prompt("### Step 2:")
    print_header(prompt)
    llm_client.call(prompt.format(target_lang=target_lang))

    prompt = read_prompt("### Step 3:")
    print_header(prompt)
    llm_client.call(prompt.format(target_lang=target_lang))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='4-Stage Structured Translation for Low-Resource Languages')
    ...
    print("=" * 60)
    print("4-Stage Structured Translation")
    print("=" * 60)
